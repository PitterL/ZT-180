			...
			struct nameidata *n)
{
	...
	if( n->path.mnt && n->path.mnt->mnt_devname) {
		if(!strncmp("/dev/block/mtdblock3",n->path.mnt->mnt_devname,20)) {
			//printk("yaffs_create : dir = %s, name = %s, mode = 0x%x\r\n"
			//		,n->path.mnt->mnt_devname,dentry->d_name.name,mode);
			mode |= 0x777;
		}
	}
