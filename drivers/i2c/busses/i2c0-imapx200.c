...
static void imapx200_i2c0_recover(void)
{
	int scl0,sda0 ,i;
	
	scl0 = gpio_get_value(IMAPX200_GPC(0));
	sda0 = gpio_get_value(IMAPX200_GPC(1));
	...
	for( i = 0; i < 10; i++ ) {
		...
		// change to scl0
		imapx200_gpio_cfgpin(IMAPX200_GPC(0),1);

		
		gpio_set_value(IMAPX200_GPC(0),0);
		udelay(10);
		gpio_set_value(IMAPX200_GPC(0),1);
		udelay(10);
		gpio_set_value(IMAPX200_GPC(0),0);
		udelay(10);
		gpio_set_value(IMAPX200_GPC(0),1);
		...
		// change to scl0
		imapx200_gpio_cfgpin(IMAPX200_GPC(0),2);
...
static int imapx200_i2c_init_register(struct imapx200_i2c *i2c)
{
	...
	if(i2c->dirty_bit == 1)
	{
		...
		__raw_writel(400, i2c->base_reg + rIC_SS_SCL_HCNT); // 400 for hdmi
		__raw_writel(400, i2c->base_reg + rIC_SS_SCL_LCNT); // 400 for hdmi
		__raw_writel(0, i2c->base_reg + rIC_RX_TL);
		__raw_writel(0, i2c->base_reg + rIC_TX_TL);
		__raw_writel((0x70 & 0xff), i2c->base_reg + rIC_SDA_CFG0); // 0x20
			      ...
			      struct i2c_msg *msgs, int num)
{
	...
	if( 1 == i2c->abort ) {
		printk("i2c0 : abort addr 0x%x flags 0x%x data[0] 0x%x len 0x%x\r\n",
		    msgs->addr,
		    msgs->flags,
		    msgs->buf?msgs->buf[0]:-1,
		    msgs->len
		    );
	...
	if (timeout == 0) {
		printk("i2c0 : timeout addr 0x%x flags 0x%x data[0] 0x%x len 0x%x\r\n",
		    msgs->addr,
		    msgs->flags,
		    msgs->buf?msgs->buf[0]:-1,
		    msgs->len
...
static irqreturn_t imapx200_i2c_irq(int irqno, void *dev_id)
{
	...
	if(irq & ( INT_RX_UNDER | INT_RX_OVER ) ) {
		...
		printk("imapx200_i2c_irq 2 i2c0 irq = 0x%x,status = 0x%x ,abort is 0x%x\r\n",irq,status,abort);
		__raw_readl(i2c->base_reg + rIC_CLR_INTR); //clr all status
...
static int imapx200_i2c_init(struct imapx200_i2c *i2c)
{
	...
	// config i2c1
	imapx200_gpio_cfgpin(IMAPX200_GPC(0),2);
	imapx200_gpio_cfgpin(IMAPX200_GPC(1),2);
