...
static int usbhid_parse(struct hid_device *hid)
{
	...
	if( 0x19d2 == le16_to_cpu(dev->descriptor.idVendor)  // ZTE K3765-Z
						&& 0xf006 == le16_to_cpu(dev->descriptor.idProduct) ){	
		return -ENODEV;
	}
	...
	printk("usbhid_parse : hdesc->bNumDescriptors = 0x%x\r\n",hdesc->bNumDescriptors);
...
static int usbhid_start(struct hid_device *hid)
{
	...
	printk("usbhid_start : interface->desc.bNumEndpoints = 0x%x\r\n",interface->desc.bNumEndpoints);
