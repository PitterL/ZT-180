...
/*
static void sdhci1_dumpregs(struct sdhci_host *host)
{
...
static void sdhci_set_card_detection(struct sdhci_host *host, bool enable)
{
	...
	if (host->quirks & (SDHCI_QUIRK_BROKEN_CARD_DETECTION|SDHCI_QUIRK_FIXED_CARD_DETECTION))
		return;
...
static u8 sdhci_calc_timeout(struct sdhci_host *host, struct mmc_data *data)
{
	...
	if (count >= 0xF) {
	    /*
		printk(KERN_WARNING "%s: Too large timeout(0x%x) requested!\n",
...
static void sdhci_send_command(struct sdhci_host *host, struct mmc_command *cmd)
{
    ...
    /*
    printk("%s cmd %d arg 0x%x flags 0x%x \n",
    ...
    */
	...
	mod_timer(&host->timer, jiffies + /*10*/timeout * HZ);
...
static void sdhci_timeout_timer(unsigned long data)
{
	...
	if (host->mrq) {
        ...
        if (host->cmd){
            printk(KERN_ERR "%s: Timeout cmd %d arg 0x%x\n",
                mmc_hostname(host->mmc),host->cmd->opcode,host->cmd->arg);
        }else{
            printk(KERN_ERR "%s: Timeout mrq cmd %d arg 0x%x\n",
                mmc_hostname(host->mmc),host->mrq->cmd->opcode,host->mrq->cmd->arg);            
        }
...
static void sdhci_cmd_irq(struct sdhci_host *host, u32 intmask)
{
	...
	if (host->cmd->error) {
		printk("%s: cmd %d error arg 0x%x resp0 0x%x int = 0x%x \n",
			mmc_hostname(host->mmc),
			host->cmd->opcode,
			host->cmd->arg,
			host->cmd->resp[0],
...
static void sdhci_data_irq(struct sdhci_host *host, u32 intmask)
{
	...
	if (intmask & SDHCI_INT_DATA_TIMEOUT){
	    ...
	    printk(KERN_ERR "%s: data crc / end bit failed\n", mmc_hostname(host->mmc));
...
int sdhci_add_host(struct sdhci_host *host)
{
    ...
    //printk("caps 0x%x mmc->caps =0x%x\n",caps,mmc->caps);
