...
#if defined(CONFIG_SDI1_USE_EPLL_CLOCK)
				  ...
				  struct sdhci_host *host)
{
    ...
    printk("IMAP_SDHCI_CONTROL2 0x%x 0x%x\n",host->ioaddr,ctrl2);
...
static int __devinit sdhci_imap_probe(struct platform_device *pdev)
{
    ...
    //card insterted always
    host->quirks |= SDHCI_QUIRK_FIXED_CARD_DETECTION;
